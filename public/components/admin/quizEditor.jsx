  ...
  render() {
    return (
      <div className='col-xs-12' >
        <div className='row'>
          <button className='btn btn-primary btn-block' type='button' onClick={() => { this.save(); }}>
            Save Quiz
          </button>
        </div>
        <div className='row'>
          ...
          <div>
            {
              this.state.answers.map((answer, index) => {
                const els = (
                  <button className='btn btn-primary btn-xs' type='button' data-index={index} onClick={(evt) => { this.showAnswerText($(evt.target).attr('data-index')); }}>
                    {answer.substr(0, 10) + (answer.length > 10 ? '...' : '')}
                    &ensp;
                    <span className='badge' onClick={(evt) => { this.removeAnswer(evt, $(evt.target).attr('data-index')); }}>
                      x
                    </span>
                  </button>
                );
